...
export class NativeFileWrapper implements FileObject {
    ...
    private execPlugin(action: string, args: any[] = []): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            cordova.exec(
                ...
                (error: any) => {
                    console.error(`[NativeFileWrapper] execPlugin error: action=${action}, error=${JSON.stringify(error)}`);
                    reject(error);
                },
                'nativeFile',
                action,
    ...
    async childByNameExists(name: string): Promise<boolean | null> {
        try {
            ...
            console.error(`[childByNameExists] path=${this.path}, name=${name}, error=${error}`);
            return null;
        }
    }
    ...
    async getChildByName(name: string): Promise<FileObject | null> {
        try {
            ...
            console.error(`[getChildByName] path=${this.path}, name=${name}, error=${error}`);
            return null;
        }
    }
