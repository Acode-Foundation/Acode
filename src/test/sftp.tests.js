...
export async function runSftpTests(writeOutput) {
	const runner = new TestRunner("SFTP API Tests");

	const testDir = window.SFTP_TEST_URL;

	runner.test("SFTP_TEST_URL is defined", (test) => {
		if (!testDir) {
			throw new SkipTest(
				"window.SFTP_TEST_URL is not defined. Set it to run SFTP tests.",
			);
		}
		test.assert(
			typeof testDir === "string",
			"SFTP_TEST_URL should be a string",
		);
		test.assert(
			testDir.startsWith("sftp://"),
			"SFTP_TEST_URL should start with sftp://",
		);
	});

	runner.test("fsOperation returns a FileSystem object for SFTP", (test) => {
		if (!testDir) throw new SkipTest("No SFTP_TEST_URL");
	...
	runner.test(
		"SFTP: createFile, exists, writeFile, readFile, delete",
		async (test) => {
			if (!testDir) throw new SkipTest("No SFTP_TEST_URL");

			const fs = fsOperation(testDir);
			const filename = `__sftp_test_${Date.now()}__.txt`;
			const fileUrl = Url.join(testDir, filename);

			try {
				// 1. Create the file
				const createdUrl = await fs.createFile(filename, "sftp test content");
				test.assertEqual(
					createdUrl,
					fileUrl,
				...
				test.assertEqual(exists, true, "Created file should exist on SFTP");
				...
				// 4. Write new content
				await fileFs.writeFile("updated sftp content");
				const updatedContent = await fileFs.readFile("utf-8");
				test.assertEqual(
					updatedContent,
					"updated sftp content",
	...
	runner.test(
		"SFTP: createDirectory, lsDir, delete directory",
		async (test) => {
			if (!testDir) throw new SkipTest("No SFTP_TEST_URL");

			const fs = fsOperation(testDir);
			const dirname = `__sftp_dir_test_${Date.now()}__`;
