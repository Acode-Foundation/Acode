...
class WebSocketInstance extends EventTarget {
	constructor(url, instanceId, binaryType) {
		...
		exec(
			(event) => {
				logIfDebug(
					`[Cordova WebSocket - ID=${this.instanceId}] Event from native:`,
					event,
				);
				...
				if (event.type === "message") {
					...
					logIfDebug(
						`[Cordova WebSocket - ID=${this.instanceId}] msg Event:`,
						event,
						msgData,
					);
					const msgEvent = new MessageEvent("message", { data: msgData });

					Object.defineProperty(msgEvent, "binary", {
						enumerable: true,
						value: event.isBinary,
					});
	...
	send(message, binary) {
		...
		if (message instanceof ArrayBuffer || ArrayBuffer.isView(message)) {
			...
			exec(
				() =>
					logIfDebug(
						`[Cordova WebSocket - ID=${this.instanceId}] Sent message(binary payload):`,
						finalMessage,
					),
				(err) =>
					console.error(
						`[Cordova WebSocket - ID=${this.instanceId}] Send error:`,
						err,
					),
				"WebSocketPlugin",
				"send",
				[this.instanceId, finalMessage, binary],
			);
			...
			exec(
				() =>
					logIfDebug(
						`[Cordova WebSocket - ID=${this.instanceId}] Sent message(binary=${binary}):`,
						finalMessage,
					),
				(err) =>
					console.error(
	...
	close(code, reason) {
		...
		exec(
			() =>
				logIfDebug(
					`[Cordova WebSocket - ID=${this.instanceId}] Close requested`,
					code,
					reason,
				),
			(err) =>
				console.error(
					`[Cordova WebSocket - ID=${this.instanceId}] Close error`,
					err,
				),
			"WebSocketPlugin",
			"close",
...
const connect = function (url, protocols = null, headers = null, binaryType) {
	return new Promise((resolve, reject) => {
		exec(
			(instanceId) => resolve(new WebSocketInstance(url, instanceId)),
			reject,
			"WebSocketPlugin",
			"connect",
			[url, protocols, binaryType, headers],
		);
	});
};
...
const send = function (instanceId, message, binary) {
	return new Promise((resolve, reject) => {
		if (typeof message === "string") {
			...
			exec(resolve, reject, "WebSocketPlugin", "send", [
				instanceId,
				message,
				binary,
			]);
...
const close = function (instanceId, code, reason) {
	return new Promise((resolve, reject) => {
		exec(resolve, reject, "WebSocketPlugin", "close", [
			instanceId,
			code,
			reason,
		]);
