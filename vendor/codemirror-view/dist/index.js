...
*/
const viewportBuffer = /*@__PURE__*/Facet.define({
    combine(values) {
        if (!values.length)
            return null;
        let value = values[0];
...
*/
const controlledTouchScroll = /*@__PURE__*/Facet.define({
    combine(values) {
        if (!values.length)
            return null;
        let value = values[0];
...
@internal
*/
function applyViewportBuffer(marginTop, marginBottom, maxAhead, direction, active, editorHeight, scaleY) {
    if (active && direction) {
        let ahead = Math.max(1000 /* VP.Margin */ / 2, editorHeight * maxAhead * scaleY);
        if (direction < 0)
            marginTop = Math.max(marginTop, ahead);
        else
            marginBottom = Math.max(marginBottom, ahead);
    }
...
class ViewState {
    ...
    activateViewportBuffer(delta) {
        let config = this.state.facet(viewportBuffer);
        if (!config || Math.abs(delta) <= 1)
            return null;
    ...
    setControlledScrollCorridor(from, to) {
        let corridor = {
            from: Math.max(0, Math.min(from, this.docHeight)),
            to: Math.max(0, Math.min(to, this.docHeight))
        };
        if (corridor.to < corridor.from)
            [corridor.from, corridor.to] = [corridor.to, corridor.from];
    ...
    viewportMargins(bias) {
        let marginTop = (0.5 - Math.max(-0.5, Math.min(0.5, bias / 1000 /* VP.Margin */ / 2))) * 1000 /* VP.Margin */;
        let marginBottom = 1000 /* VP.Margin */ - marginTop;
        let config = this.state.facet(viewportBuffer);
        let margins = applyViewportBuffer(marginTop, marginBottom, (config === null || config === void 0 ? void 0 : config.maxAhead) || 0, this.scrollDirection, !!config && Date.now() < this.viewportBufferUntil, this.editorHeight, this.scaleY);
        if (this.controlledScrollCorridor) {
    ...
    getViewport(bias, scrollTarget) {
        ...
        let viewport = new Viewport(map.lineAt(visibleTop - margins.top, QueryType.ByHeight, oracle, 0, 0).from, map.lineAt(visibleBottom + margins.bottom, QueryType.ByHeight, oracle, 0, 0).to);
        ...
        if (scrollTarget) {
            ...
            if (head < viewport.from || head > viewport.to) {
                ...
                viewport = new Viewport(map.lineAt(topPos - margins.top, QueryType.ByHeight, oracle, 0, 0).from, map.lineAt(topPos + viewHeight + margins.bottom, QueryType.ByHeight, oracle, 0, 0).to);
            }
        }
        return viewport;
    }
    ...
    viewportIsAppropriate({ from, to }, bias = 0) {
        ...
        let topCover = active && this.scrollDirection < 0 ? margins.top : Math.max(10 /* VP.MinCoverMargin */, Math.min(-bias, 250 /* VP.MaxCoverMargin */));
        let bottomCover = active && this.scrollDirection > 0 ? margins.bottom : Math.max(10 /* VP.MinCoverMargin */, Math.min(bias, 250 /* VP.MaxCoverMargin */));
        return (from == 0 || top <= visibleTop - topCover) &&
            (to == this.state.doc.length ||
                bottom >= visibleBottom + bottomCover) &&
            (top > visibleTop - 2 * Math.max(1000 /* VP.Margin */, margins.top) &&
                bottom < visibleBottom + 2 * Math.max(1000 /* VP.Margin */, margins.bottom));
    }
...
@internal
*/
function isNativeScrollActivity(touchActive, lastTouchTime, lastWheelEvent, now) {
...
class DOMObserver {
    ...
    onScrollChanged(e) {
        ...
        if ((_a = this.view.controlledTouchScrollController) === null || _a === void 0 ? void 0 : _a.onScrollChanged())
            return true;
        let input = this.view.inputState, now = Date.now();
        let nativeScroll = isNativeScrollActivity(input.touchActive, input.lastTouchTime, input.lastWheelEvent, now);
    ...
    onScroll(e) {
        ...
        if (this.editContext && !controlled)
            this.view.requestMeasure(this.editContext.measureReq);
    }
    onResize() {
        var _a;
        (_a = this.view.controlledTouchScrollController) === null || _a === void 0 ? void 0 : _a.cancel();
 ...
 */
const DecelerationRate = /*@__PURE__*/Math.log(.78) / /*@__PURE__*/Math.log(.9);
const Inflexion = .35;
...
const AndroidMinimumFlingVelocity = 50;
const AndroidMaximumFlingVelocity = 8000;
const SplinePosition = /*@__PURE__*/(() => {
    let result = new Array(SampleCount + 1), xMin = 0;
    for (let i = 0; i < SampleCount; i++) {
        ...
        for (;;) {
            ...
            if (Math.abs(position - alpha) < 1e-5)
                break;
            if (position > alpha)
                xMax = x;
            else
...
*/
function estimateCommittedVelocity(samples, now, windowMs = 80, fallbackWindowMs = 160) {
...
function createAndroidSplineFling(velocity) {
    if (!Number.isFinite(velocity) || Math.abs(velocity) < AndroidMinimumFlingVelocity)
        return null;
    let initialVelocity = Math.round(Math.max(-AndroidMaximumFlingVelocity, Math.min(AndroidMaximumFlingVelocity, velocity)));
...
function sampleAndroidSpline(fling, elapsed) {
...
const hasPendingInput = typeof navigator != "undefined" && ((_a = navigator.scheduling) === null || _a === void 0 ? void 0 : _a.isInputPending)
    ? () => navigator.scheduling.isInputPending() : () => false;
/**
@internal
*/
function claimControlledTouchGesture(deltaX, deltaY, threshold = GestureThreshold, axisRatio = AxisRatio) {
...
@internal
*/
class ControlledTouchScroll {
    constructor(view, supported = browser.android && browser.chrome) {
        this.view = view;
        ...
        this.preparationChunkScreens = InitialPreparationScreens;
        this.fastPreparations = 0;
        this.generation = 0;
        this.highlightCoverageManaged = false;
        this.highlightCoverageReady = false;
        this.highlightFrom = 0;
        this.highlightTo = 0;
        this.highlightRefreshTimer = -1;
        this.gestureStarted = 0;
        this.firstMovementDelay = 0;
        this.momentum = false;
        this.momentumFrame = -1;
        this.fling = null;
        this.momentumStarted = 0;
        this.momentumElapsed = 0;
        this.momentumDistance = 0;
        this.momentumStartTop = 0;
        this.lastMomentumCommit = 0;
        this.releaseVelocity = 0;
        this.expectedMomentumDistance = 0;
        this.expectedMomentumDuration = 0;
        ...
        this.onTouchStart = (event) => {
            ...
            this.pendingDelta = 0;
            this.direction = 0;
            this.samples = [{ at: this.now(), top: this.view.scrollDOM.scrollTop }];
            this.releaseVelocity = 0;
        ...
        this.onTouchMove = (event) => {
            ...
            if (Math.abs(delta) > .1) {
                ...
                if (justClaimed && target >= bounds.from && target <= bounds.to)
                    this.applyMovement();
                else
        ...
        this.onTouchEnd = (event) => {
            ...
            if (!this.claimed) {
                ...
                this.samples = [];
                if (this.corridorMode != "idle")
                    this.scheduleSettle((_b = (_a = this.config) === null || _a === void 0 ? void 0 : _a.settleDelay) !== null && _b !== void 0 ? _b : 0);
                return;
            }
            if (this.claimed && event.cancelable)
                event.preventDefault();
            this.tracking = false;
        ...
        this.onTouchCancel = () => this.cancel();
        this.onScrollEnd = () => {
            var _a, _b;
        ...
        this.onCompositionStart = () => this.cancel();
        this.onResize = () => this.cancel();
        this.applyMovement = () => {
            this.movementFrame = -1;
            if (!this.claimed)
                return;
        ...
        this.startIdlePrewarm = () => {
            ...
            if (height <= 0) {
                this.scheduleIdlePrewarm((_b = (_a = this.config) === null || _a === void 0 ? void 0 : _a.settleDelay) !== null && _b !== void 0 ? _b : 0);
                return;
            }
        ...
        this.applyMomentum = () => {
            ...
            let delta = Math.sign(rawDelta) * Math.min(Math.abs(rawDelta), maximumStep);
            if (frameTime > MaximumMomentumFrame + .5)
    ...
    */
    get renderCritical() {
        return this.tracking || this.claimed || this.momentum ||
            (this.corridorMode == "active" && this.preparing) ||
            this.movementFrame > -1 || this.momentumFrame > -1;
    }
    beforeUpdate(transactions) {
        if (transactions.some(tr => tr.docChanged || tr.reconfigured))
            this.cancel();
    }
    update(update) {
    ...
    */
    clampToRenderedOffset(target) {
        var _a;
        if (!this.enabled || !this.ownsScroll)
            return target;
        let bounds = this.movementBounds();
        let pinned = (_a = this.controlledTop) !== null && _a !== void 0 ? _a : target;
    ...
    */
    ...
    now() {
        var _a, _b;
        return (_b = (_a = this.view.win.performance) === null || _a === void 0 ? void 0 : _a.now()) !== null && _b !== void 0 ? _b : Date.now();
    }
    syncConfig() {
        if (this.destroyed)
            return;
    ...
    unbindResize() {
        var _a;
        (_a = this.resizeWindow) === null || _a === void 0 ? void 0 : _a.removeEventListener("resize", this.onResize);
        this.resizeWindow = null;
    }
    activateCorridor() {
        this.corridorMode = "active";
        this.idlePrewarmReady = false;
        if (this.view.viewState.controlledScrollActive)
            return;
        let top = this.view.scrollDOM.scrollTop, height = this.view.scrollDOM.clientHeight;
        this.corridorFrom = top;
        this.corridorTo = top + height;
    }
    setCorridor(prepare = true) {
        let scale = this.view.viewState.scaleY || 1;
        let max = (this.view.scrollDOM.scrollHeight + this.view.scrollDOM.clientHeight) * scale;
        let changed = this.view.viewState.setControlledScrollCorridor(Math.max(0, this.corridorFrom * scale), Math.min(max, this.corridorTo * scale));
        if (changed && prepare)
            this.requestPreparation();
    }
    requestPreparation() {
        if (this.preparing || this.destroyed)
            return;
        this.preparing = true;
        ...
        this.view.requestMeasure({
            ...
            write: measuredGeneration => {
                if (this.destroyed || measuredGeneration != this.generation)
                    return;
                this.preparing = false;
    ...
    scheduleMovement() {
        if (this.movementFrame > -1 || this.destroyed)
            return;
        this.movementFrame = this.view.win.requestAnimationFrame(this.applyMovement);
    }
    stopMovementFrame() {
        if (this.movementFrame > -1)
            this.view.win.cancelAnimationFrame(this.movementFrame);
        this.movementFrame = -1;
    }
    recordSample(top) {
        let now = this.now();
    ...
    movementBounds() {
        let bounds = this.view.viewState.controlledScrollBounds();
        if (!this.highlightCoverageManaged)
            return bounds;
    ...
    scheduleHighlightCoverageRefresh() {
        var _a, _b;
        if (!this.highlightCoverageManaged || this.destroyed)
            return;
        this.clearHighlightCoverageRefresh();
        this.highlightRefreshTimer = setTimeout(() => {
            ...
            if (!this.destroyed && !this.tracking && !this.claimed && !this.momentum)
                this.view.dispatch({});
        }, ((_b = (_a = this.config) === null || _a === void 0 ? void 0 : _a.settleDelay) !== null && _b !== void 0 ? _b : 0) + 1);
    }
    clearHighlightCoverageRefresh() {
        if (this.highlightRefreshTimer > -1)
            clearTimeout(this.highlightRefreshTimer);
        this.highlightRefreshTimer = -1;
    }
    recordPreparation(duration) {
        this.preparationDuration = Math.max(0, duration);
        if (duration > SlowPreparation) {
            this.preparationChunkScreens = Math.max(MinimumPreparationScreens, this.preparationChunkScreens / 2);
            this.fastPreparations = 0;
        }
        else if (duration <= PreparationBudget) {
            if (++this.fastPreparations >= 2) {
                this.preparationChunkScreens = Math.min(MaximumPreparationScreens, this.preparationChunkScreens * 1.5);
                this.fastPreparations = 0;
            }
        }
        else {
    ...
    scheduleIdlePrewarm(delay) {
        if (!this.enabled || this.destroyed)
            return;
        this.clearPrewarmTimer();
        this.prewarmTimer = setTimeout(this.startIdlePrewarm, Math.max(0, delay) + 1);
    }
    clearPrewarmTimer() {
        if (this.prewarmTimer > -1)
            clearTimeout(this.prewarmTimer);
        this.prewarmTimer = -1;
    }
    scheduleIdleContinuation() {
        if (this.prewarmTimer > -1 || this.destroyed || this.corridorMode != "idle")
            return;
    ...
    continueIdlePrewarm() {
        if (this.corridorMode != "idle" || this.destroyed || this.tracking ||
            this.claimed || this.momentum || this.preparing)
            return;
    ...
    maybeExtendCorridor() {
        if (!this.config || this.preparing || this.tracking || hasPendingInput())
            return;
        let scroller = this.view.scrollDOM, height = scroller.clientHeight;
        if (this.remainingSafeDistance() > height)
            return;
    ...
    scheduleMomentum() {
        if (!this.momentum || this.momentumFrame > -1 || this.destroyed)
            return;
        this.momentumFrame = this.view.win.requestAnimationFrame(this.applyMomentum);
    }
    finishMomentum(reason) {
        var _a, _b;
        this.stopMomentum(reason);
        this.scheduleSettle((_b = (_a = this.config) === null || _a === void 0 ? void 0 : _a.settleDelay) !== null && _b !== void 0 ? _b : 0);
    }
    stopMomentum(reason) {
        if (this.momentumFrame > -1)
            this.view.win.cancelAnimationFrame(this.momentumFrame);
        this.momentumFrame = -1;
    ...
    scheduleSettle(delay) {
        this.clearSettleTimer();
        this.settleTimer = setTimeout(this.settle, delay + 1);
    }
    clearSettleTimer() {
    ...
    releaseCorridor(requestMeasure = true) {
        this.clearSettleTimer();
        this.preparing = false;
        this.controlledTop = null;
        this.corridorMode = "none";
        ...
        this.generation++;
        if (this.view.viewState.clearControlledScrollCorridor() && requestMeasure)
            this.view.requestMeasure();
    }
}
                            ...
                            if ((diff > 1 || diff < -1) &&
                                ...
                                if (!scroll)
                                    ...
                                    scroll.scrollTop = scroll == this.scrollDOM
                                        ? (_b = (_a = this.controlledTouchScrollController) === null || _a === void 0 ? void 0 : _a.clampToRenderedOffset(target)) !== null && _b !== void 0 ? _b : target
                                        : target;
                                }
                                scrollAnchorHeight = -1;
                                continue;
...
*/
EditorView.viewportBuffer = viewportBuffer;
/**
Configure render-gated vertical touch scrolling on Android Chromium.
*/
EditorView.controlledTouchScroll = controlledTouchScroll;
/**
...
const __test = { HeightMap, HeightOracle, MeasuredHeights, QueryType, ChangedRange, computeOrder,
    ...
    applyViewportBuffer, isNativeScrollActivity, claimControlledTouchGesture,
    AndroidMinimumFlingVelocity, AndroidMaximumFlingVelocity,
    createAndroidSplineFling, estimateCommittedVelocity, sampleAndroidSpline,
