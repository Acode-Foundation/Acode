...
const viewportPriorityParsingFacet = state.Facet.define({
    combine(values) {
        if (!values.length)
            return null;
        let value = values[0];
...
class ParseWorker {
    ...
    restartProvisional(delay = 0) {
        ...
        try {
            ...
            view.logException(this.view.state, error, "viewport-priority parse");
        }
    }
...
const parseWorker = view.ViewPlugin.fromClass(ParseWorker, {
...
class TreeHighlighter {
    ...
    buildDeco(view$1, highlighters) {
        if (!highlighters)
            return view.Decoration.none;
        let builder = new state.RangeSetBuilder();
        let provisional = viewportPriorityTree(view$1);
    ...
    addTreeHighlights(builder, tree, highlighters, from, to, offset = 0) {
        highlight.highlightTree(tree, highlighters, (from, to, style) => {
            builder.add(from + offset, to + offset, this.markCache[style] || (this.markCache[style] = view.Decoration.mark({ class: style })));
        }, from, to);
    }
}
/**
...
exports.viewportPriorityParsing = viewportPriorityParsing;
