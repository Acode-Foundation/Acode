...
## ADR-006: Distribution, monetization, and minimum device floor
- **Date:** 2026-09-25
- **Status:** accepted
- **Decision:** Distribute via Google Play, F-Droid, and GitHub Releases (all three, not a subset). Monetize as freemium: the app itself stays free, with a paid Pro tier for advanced features (scope TBD — see consequence below). Minimum supported Android version is Android 13 (API 33) — modern-only, no support for older devices.
- **Alternatives considered:** F-Droid/GitHub only (narrower reach); fully free/no monetization and donations-only (both rejected in favor of a Pro-tier funding path); a broader device floor at API 26 or API 29 (BLUEPRINT.md's original "mid-range, not flagship" framing) — rejected in favor of a modern-only floor.
- **Why:** Project owner's explicit call, made when the other options were laid out directly.
- **Consequence:** Freemium + F-Droid is a real tension worth flagging now: F-Droid's build/signing pipeline doesn't support Play Billing or other proprietary IAP paths, so the F-Droid build likely needs its own story (e.g. F-Droid ships the full feature set free, or gates Pro features behind a license file/key entered manually, rather than in-app purchase). Acode's own existing build tooling already has a `paid`/`fdroid` flavor split (see `nightly-build.yml`, now removed, and `npm run build paid ... fdroid`) worth checking during the Phase 0.2 scaffold for reusable patterns. What's actually gated behind Pro (which agent/IDE features are free vs. paid) is a separate, later decision — not resolved here. The API 33 floor updates BLUEPRINT.md §7's "target mid-range devices for performance budgets, not flagships": performance budgets still target mid-range hardware, just on the Android 13+ OS baseline, not older OS versions.
- **Still open (deferred, non-blocking for Phase 0/1):** team size, and the first three model providers to support — both revisited before Phase 1's agent v1 work starts, since neither affects the Phase 0 shell migration.
